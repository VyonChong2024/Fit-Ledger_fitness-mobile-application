...
class DietPlanTypeFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        // Clear lists to prevent duplicate references when fragment is recreated
        ...
        // Restore selected diet from ViewModel if any
        selectedCards = viewModel.dietPlan
        updateButton()
        ...
        for (i in 1..cardCount) {
            ...
            if (selectedCards == frontTexts[index].text.toString()) {
                flipToBackWithoutAnimation(index)
            } else {
                flipToFrontWithoutAnimation(index)
            }
        }
    ...
    // Helpers to restore state without animation
    private fun flipToBackWithoutAnimation(index: Int) {
        frontCards[index].visibility = View.GONE
        backCards[index].visibility = View.VISIBLE
        isFrontVisible[index] = false
    }

    private fun flipToFrontWithoutAnimation(index: Int) {
        backCards[index].visibility = View.GONE
        frontCards[index].visibility = View.VISIBLE
