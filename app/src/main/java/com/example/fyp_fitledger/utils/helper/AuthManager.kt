    ...
    fun handleSignIn(result: GetCredentialResponse, context: Context, callback: (Boolean, String?) -> Unit) {
        ...
        if (credential is CustomCredential && credential.type == GoogleIdTokenCredential.TYPE_GOOGLE_ID_TOKEN_CREDENTIAL) {
            try {
                ...
                if (idToken != null) {
                    Log.d("SignIn", "Retrieved Google ID token: $idToken")
        ...
        auth.signInWithCredential(credential)
            .addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    ...
                    Log.e("SignIn", "Firebase sign-in failed", exception)

                    // Extra logging for debugging
                    exception?.let {
                        Log.e("SignIn", "Exception class: ${it::class.java}")
                        Log.e("SignIn", "Exception message: ${it.message}")
                        if (it is FirebaseAuthInvalidCredentialsException) {
                            Log.e("SignIn", "Status code: ${it.errorCode}")
                        }
                    }
